...
HJHInstrSynthDef : SynthDef {
	...
	*build { arg instr,args,outClass = \Out,lagMagic;
		^super.prNew.build(instr,args,outClass,lagMagic)
	}
	build { arg instr,args,outClass= \Out,argLagMagic;
		// this.dumpBackTrace;
		// "^^ debug trace".debug;
	...
	buildUgenGraph { arg argInstr,args,outClass;
...

		{
			...
			HJHInstrSynthDef.clearCache(Server.default);
	...
	playerIn { arg object;
		...
		^object.hjhInstrArgFromControl(bus);
	}
...
//	}
	...
	buildControlsWithObjects { arg instr,objects;
		...
		^outputProxies.collect({ arg outp,i;
			defargs.at(i).hjhInstrArgFromControl(outp, i, lagMagic)
		})
	}
	...
	finishBuild {
		...
		outputProxies = nil;
		instr = nil;
		("HJHInstrSynthDef built:" + name).inform;
	}
	buildErrorString {
