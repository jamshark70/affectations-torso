...
HJHPatch : HasPatchIns  {
	...
	*new { arg name,inputs,outClass;
		...
		//[name, inputs, outClass].debug("HJHPatch *new");
	...
	asSynthDef {
		...
		^synthDef ?? {
			...
			synthDef = HJHInstrSynthDef.build(this.instr,this.args,this.outClass,this.lagMagic);
	...
	storeParamsOn { arg stream;
		var last;
		if(this.class === HJHPatch,{ // an indulgence ...
			last = args.size - 1;
