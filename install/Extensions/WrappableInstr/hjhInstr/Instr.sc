...
HJHInstr  {
	...
	*new { arg name, func, specs, outSpec;
		...
		if(previous.notNil,{
			if(previous.isKindOf(HJHInstr).not,{
	...
	*loadAll {
		var quarkInstr, ext;
		(this.dir ++ "*").pathMatch.reject { |path| path.debug;
			(ext = path.splitext[1]) == "sc" /*or: {
				ext.any { |ch| ch.isAlpha.not }
				}*/
		}.do({ arg path;
			{
				if(path.last != $/,{
	...
	*dir_ { arg p;
	...
	asSynthDef { arg args,outClass=\Out;
		var synthDef;
		synthDef = HJHInstrSynthDef.new;
		synthDef.build(this,args,outClass,this.lagMagic);
		^synthDef
	}
	...
	test { arg ... args;
		var p;
		p = HJHPatch(this.name,args);
		p.topGui;
		^p
	}
	play { arg ... args;
		^HJHPatch(this.name,args).play
	}
	...
	*leaves { arg startAt; // address array
		...
		if(dict.isNil,{
			Error("HJHInstr address not found:" + startAt.asCompileString).throw;
		});
	...
	*findFileFor { arg symbolized;
		...
		quarkInstr = (Platform.userExtensionDir ++ "/quarks/*/HJHInstr").pathMatch;
		quarkInstr.do({ |path|
		...
		(rootPath++"*").pathMatch.do({ |path|
			...
			if(file.last == $/,{
				...
				if(orcname == pathPartsFirst,{
					...
					HJHInstr.leaves(symbols).do({ |instr| instr.path = path });
					^path
				});
			});
		});
	...
	*initClass {
		...
		if(dir.isNil,{ dir = Document.dir ++ "HJHInstr/"; });
	}
